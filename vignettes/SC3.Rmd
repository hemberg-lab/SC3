...
It is easy to create an `SingleCellExperiment` object from `yan` expression matrix:
```{r}
...
After the `sce` object is created and QC is run, `scater` allows a user to quickly visualize and assess the data, for example using a PCA plot:
```{r}
...
# Note that n_cores = 1 is required for compilation of this vignette.
# Please remove this parameter when running on your computer:
# sce <- sc3_prepare(sce, ks = 2:4)
sce <- sc3_prepare(sce, ks = 2:4, n_cores = 1)
str(sce@sc3)
```
...
names(sce@sc3$consensus)
names(sce@sc3$consensus$`3`)
```
