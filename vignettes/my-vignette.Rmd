...
Note that we also provide a convenient wrapper to quickly create `SCESet` object from an R matrix: the function name is `create_sceset` and the input argument is an R matrix containing expression values. `create_sceset` combines multiple lines of code we used above (please look at its documentation `?create_sceset`):
```{r}
