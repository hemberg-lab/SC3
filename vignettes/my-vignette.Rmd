...
example_sceset <- sc3_kmeans(example_sceset, 3:5)
...
example_sceset <- sc3_calc_biology_after_svm(example_sceset)
```


```{r, message=FALSE, warning=FALSE, include=FALSE}
# The vignette can not be run on multiple cores when installing the package,
# therefore I use two chunks here to make it work
sc3_interactive(example_sceset)
```

```{r, eval=FALSE}
example_sceset <- run_sc3(example_sceset, k = 7)
```

To visualize:
```{r, eval=FALSE}
sc3_interactive(example_sceset@consensus)
```
