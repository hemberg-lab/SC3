    ...
    shinyApp(
            ...
            fluidRow(
                column(3,
                   wellPanel(
                        ...
                        if(with_svm) {
                            p("Press this button when you have found the best
                              clustering\n\n")
                        },
                        if(with_svm) {
                        ...
                        p("\n\nOpen a corresponding panel first, then press a
                          button:"),
        ...
        server = function(input, output, session) {
            ...
            output$labels <- renderUI({
                ...
                labs <- paste0("<br/><font size=\"3\">Colours correspond to
                               clusters obtained by clustering the data by <b>",
                ...
                withProgress(message = 'Calculating DE genes...', value = 0, {
                    ...
                    validate(
                        need(try(length(values$de.res) != 0),
                             "\nUnable to find significantly (p-value < 0.05)
                             differentially expressed genes from obtained
                             clusters! Please try to change the number of
                             clusters k and run DE analysis again.")
                    )
            ...
            get_mark_genes <- eventReactive(input$get_mark_genes, {
                ...
                validate(
                    need(try(!is.null(rownames(dataset))),
                         "\nNo gene names provided in the input
                         expression matrix!")
                )
                withProgress(message = 'Calculating Marker genes...',
                    ...
                    validate(
                        need(try(dim(values$mark.res)[1] != 0),
                             "\nUnable to find significant marker genes from
                             obtained clusters! Please try to change the
                             number of clusters k and run marker analysis
                             again.")
                    )
            ...
            observeEvent(input$GO, {
                validate(
                    need(try(!is.null(values$mark.res)),
                         "\nPlease run marker genes analysis by clicking
                         on \"Get Marker genes\" button!")
                )
                open_webgestalt_go(
                    rownames(
                ...
                content = function(file) {
                    ...
                    validate(
                        need(try(!is.null(values$de.res)),
                             "\nPlease run differential expression analysis by
                             clicking on \"Get DE genes\" button!")
                    )
                    write.table(
                ...
                content = function(file) {
                    ...
                    validate(
                        need(try(!is.null(values$outl.res)),
                             "\nPlease run cell outlier analysis by using
                             \"Get Cells outliers\" button!")
                    )
                    if(with_svm) {
                        write.table(
